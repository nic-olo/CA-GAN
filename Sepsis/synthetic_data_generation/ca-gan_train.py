...
def main(hp: DictConfig):
    ...
    print("Load...")
    df = pd.read_pickle(
        original_cwd + hp.dir.data + "A001_data_real_transformed.pkl")

    df = df.drop(["Admn001_ID"], axis=1)
    ethnicity_labels = pd.read_csv(
        original_cwd + hp.dir.data + "mimictable.csv")['ethnicity']
    ...
    data_types = pd.read_csv(
        original_cwd + hp.dir.data + "A001_data_types.csv",
        usecols=dtype.keys(),
        dtype=dtype,
        index_col="index",
    )
    ...
    labels = ethnicity_labels.values[::15]
