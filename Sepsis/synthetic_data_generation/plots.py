...
def main(hp: DictConfig):
    ...
    df_all["FiO2"] = df_all["FiO2"].astype(
        str).astype("category").map(mapper_FiO2)
    ...
    df_all["GCS (Total)"].cat.reorder_categories(
        mapper_GCS.values(), inplace=True)
    ...
    for col in [
        ...
        df_all[col] = df_all[col].astype(
            str).astype("category").map(mapper_bin)
