...
def Execute_C003(
    ...
    ethnicity = pd.read_csv('../data/mimictable.csv')
    ethnicity = ethnicity.drop(ethnicity[
                               (ethnicity['ethnicity'] != 1.0)].index,
                               inplace=False)
    ethnicity_ids = np.array(ethnicity['icustayid'])
    df = df.loc[(df['Admn001_ID']).isin(ethnicity_ids)]
...
df = pd.read_csv("../data/A001_data_real_transformed.csv")
...
torch.save(All_Trainable_Data, '../data/All_Trainable_Data')
