...
if TESTING:
    RUN_NAME = f"Sepsis_Runs/{data_type}/Testing Run - {datetime.datetime.now().strftime('%m-%d--%H:%M')}"
else:
    RUN_NAME = f"Sepsis_Runs/{data_type}/Run - {datetime.datetime.now().strftime('%m-%d--%H:%M')}"

time_series_length = 15
test_path = '../sepsis_black_data/test_set.csv'
generations_path = "../sepsis_black_data/generations"
n_models = 5
