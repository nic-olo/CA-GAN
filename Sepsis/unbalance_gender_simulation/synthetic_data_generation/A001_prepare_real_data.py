...
# ---
torch.save(A001_BTS_Float, data_dir + 'g_A001_BTS_Float')

torch.save(A001_BTS_nonFloat, data_dir + 'g_A001_BTS_nonFloat')
# ---
...
MyData_Types.to_csv(data_dir + 'g_A001_data_types.csv', index=False)
...
labels['Admn001_ID'] = MyData['icustayid']
labels['gender'] = MyData['gender']
labels.drop_duplicates(inplace=True)

print((labels[labels["gender"] == 1]).shape)

seed = 0
np.random.seed(seed)
# removed_labels = labels[labels['gender'] == 1].sample(frac = 0.0)
removed_labels = labels[labels['gender'] == 1].sample(frac=0.8)

labels = labels.drop(removed_labels.index)

# SAVE DATA

labels.to_csv(data_dir + 'g_A001_labels.csv', index=False)
...
MyData_Transformed = MyData_Transformed[MyData_Transformed["Admn001_ID"].isin(
    labels["Admn001_ID"])]

MyData_Transformed.to_csv(
    data_dir + 'g_A001_data_real_transformed.csv', index=False)
MyData_Transformed.to_pickle(data_dir + 'g_A001_data_real_transformed.pkl')
