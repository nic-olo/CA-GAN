...
class Generator(nn.Module):
    def __init__(self, data_types, class_size=2, embedding_dim=2):
        super().__init__()
        self.device = None
        self.embedding = nn.Embedding(class_size, embedding_dim)
        self.rnn = nn.LSTM(
            input_size=128 + embedding_dim,
            hidden_size=128,
            num_layers=3,
            batch_first=True,
            dropout=0.4,
        ...
        max_real = max(
            data_types.loc[data_types["type"] == "real", "index_end"])
        self.output_activations.append(
            lambda x: torch.sigmoid(x[..., 0:max_real]))
        for index, row in data_types.iterrows():
            if row["type"] != "real":
                idxs = row["index_start"]
                idxe = row["index_end"]
                # need to get values: https://stackoverflow.com/questions
                # /28014953/capturing-value-instead-of-reference-in-lambdas
                self.output_activations.append(
    ...
    def forward(self, x, seq_lengths, total_length, labels):
        self.device = get_device()
        # tensor = torch.zeros(x.shape[0], x.shape[1], dtype=torch.int).to(
        # self.device)
        # list(labels).insert(labels[0], 1)
        try:
            length = int(seq_lengths[0][0][0])
        except IndexError:
            length = 15
        labels = torch.cat([labels] * length, 1)

        # for i in range(0, labels.size()[0] * length, length):
        #     for j in range(1, length):
        #         if i + j
        #         list(labels).insert(labels[i], i+j)
        labels.reshape(x.size()[0], x.size()[1])
        labels = self.embedding(labels)
        x = torch.cat([x, labels], 2)
        x = nn.utils.rnn.pack_padded_sequence(
            x, lengths=seq_lengths.squeeze(), batch_first=True,
...
class Discriminator(nn.Module):
    def __init__(self, data_types, class_size=2, embedding_dim=2):
        super().__init__()
        self.device = get_device()
        self.embedding = nn.Embedding(class_size, embedding_dim).to(
            device=self.device)
        self.max_real = max(
            data_types.loc[data_types["type"] == "real", "index_end"])
        # embedding layers
        ...
        self.linear1 = nn.Linear(
            sum(data_types["embedding_size"]) + embedding_dim, 128)
        self.dropout = nn.Dropout(0.5)
        self.linear2 = nn.Linear(128, 128)
        self.rnn = nn.LSTM(
            input_size=128,
    ...
    def forward(self, x, seq_lengths, labels):
        try:
            length = int(seq_lengths[0][0][0])
        except IndexError:
            length = 15
        labels = torch.cat([labels] * length, 1).to(self.device)

        x_list = [x[..., 0: self.max_real]] + [
            f(x, embedding_layer.weight)
            for f, embedding_layer in
            zip(self.soft_embedding, self.embedding_layers)
        ]
        x = torch.cat(x_list, dim=-1).to(self.device)

        labels.reshape(x.size()[0], x.size()[1]).to(self.device)
        labels = self.embedding(labels)
        x = torch.cat([x, labels], 2)
...
class WGAN_GP:
    ...
    def generate_data(self, num_samples, labels, to_np=False):
        if self.seq_lengths is None:
            self.seq_lengths = torch.tensor(self.max_sequence_length).repeat(
                num_samples
            )
        z = torch.rand((num_samples,
                        max(self.seq_lengths), 128)).to(self.device)
        data_fake = self.G(z, self.seq_lengths,
                           self.curriculum_length,
                           labels=labels.to(self.device))
        data_fake = data_fake.data.cpu().numpy() if to_np else data_fake
        return data_fake

    def _critic_train_iteration(self, data_real, labels):
        """ """
        # Discriminator values for real and fake data
        data_real = data_real.to(self.device)
        data_fake = self.generate_data(self.batch_size, labels)
        D_real = self.D(data_real, self.seq_lengths, labels)
        D_fake = self.D(data_fake, self.seq_lengths, labels)
    ...
    def _generator_train_iteration(self, labels):
        """ """
    ...
    def _gradient_penalty(self, data_real, data_fake, labels):
    ...
    def train(self, trn_loader):
        for curriculum_step in range(len(self.curriculum_lengths)):
            ...
            for epoch in tqdm(range(self.epochs)):
                ...
                # Train
                for batch_idx, (data_real, labels) in enumerate(trn_loader):
                    batch_seq_lengths = data_real[1]
                    ...
                    if self.variable_length:
                        self.seq_lengths = (
                            (self.seq_lengths * torch.rand(
                                self.seq_lengths.shape))
                            .ceil()
                            .int()
                        )
                    ...
                    idx = idx + torch.arange(0,
                                             self.curriculum_length).unsqueeze(
                        0
                    ).unsqueeze(2)
                    idx = idx.clip(max=self.max_sequence_length - 1)
                    data_real = torch.gather(data_real, 1, idx)
                    self.num_steps += 1
                    self._critic_train_iteration(data_real, labels)
                    if self.num_steps % self.G_iter == 0:
                        self._generator_train_iteration(labels=labels)
                # Log losses
                ...
                if (mlflow_step % 10) == 0:
                    ...
                    mlflow.log_metric("correlation_loss",
                                      self.correlation_loss, step=mlflow_step)
    ...
    def save_model(self):
        ...
        uri_discriminator = "models_discriminator/" + str(
            self.curriculum_length)
