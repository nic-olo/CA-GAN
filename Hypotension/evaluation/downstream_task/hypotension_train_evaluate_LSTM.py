...
parser.add_argument('--data-type', type=str, choices=['Fake', 'Merged', 'Real'], help='Set the type of data', required=True)
...
if TESTING:
    RUN_NAME = f"Hypotension_Runs/{data_type}/Testing Run - {datetime.datetime.now().strftime('%m-%d--%H:%M')}"
else:
    RUN_NAME = f"Hypotension_Runs/{data_type}/Run - {datetime.datetime.now().strftime('%m-%d--%H:%M')}"


test_file = '../hypotension_black_data/test_set.csv'
generations_path = "../hypotension_black_data/generations"

n_steps = 20
n_models = 5

if TESTING or data_type == "Real":
    n_models = 1
...
if __name__ == '__main__':
    ...
    for generation in tqdm(os.listdir(generations_path)):
        ...
        if data_type == "Fake":
            file_name = "data_fake.csv"
        elif data_type == "Merged":
            file_name = "data_merged.csv"
        elif data_type == "Real":
            file_name = "data_real.csv"
        else:
        ...
        for model_num in range(n_models):
            ...
            for column in train.columns:
                model_train(train, column, generation, model_num, order_patients, n_steps)
                if TESTING:
                    break

        # Evaluation
        test = pd.read_csv(test_file)
        test.drop(['icustay_id', 'hour'], axis='columns', inplace=True)
    ...
    print("Saved at:", RUN_NAME + "/gen_column_avg_errors.csv")
