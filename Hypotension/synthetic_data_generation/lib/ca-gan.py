...
class Generator(nn.Module):
    ...
    def forward(self, x, seq_lengths, total_length, labels):
        ...
        labels = self.embedding(labels).to(self.device)
...
class Discriminator(nn.Module):
    def __init__(self, data_types, class_size=2, embedding_dim=2):
        super().__init__()
        self.embedding = nn.Embedding(class_size, embedding_dim).to("cuda:0")
        ...
        # others
        self.linear1 = nn.Linear(sum(data_types["embedding_size"]) + embedding_dim, 128)
        self.dropout = nn.Dropout(0.5)
        self.linear2 = nn.Linear(128, 128)
        self.rnn = nn.LSTM(
            input_size=128,
    ...
    def forward(self, x, seq_lengths, labels):
        ...
        try:
            ...
            length = 48
        labels = torch.cat([labels] * length, 1).to("cuda:0")
        ...
        labels = self.embedding(labels).to("cuda:0")
        x = torch.cat([x, labels], 2)
...
class WGAN_GP:
    ...
    def generate_data(self, num_samples, labels, to_np=False):
        ...
        data_fake = self.G(z, self.seq_lengths,
                           self.curriculum_length, labels=labels.to(self.device))
