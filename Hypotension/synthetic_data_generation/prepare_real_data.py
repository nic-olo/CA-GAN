...
def main(hp: DictConfig):
    ...
    df["FiO2"] = pd.cut(df["FiO2"], right=False,
                        bins=bins_FiO2, labels=bins_FiO2[:-1])
        ...
        if row["include"]:
            ...
            if row["type"] != "real" and not plotted:
                ...
                for i_plt, r_plt in data_types.iterrows():
                    if r_plt["include"]:
                        ax[i_plt].hist(df[r_plt["name"]].values,
            ...
            if row["type"] == "cat" or row["type"] == "bin":
                ...
                df = pd.get_dummies(
                    df, prefix=row["name"], columns=[row["name"]])
    ...
    data_types.to_csv(hp.dir.config + "data_types.csv",
